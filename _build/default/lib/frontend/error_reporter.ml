...
let with_note diagnostic span message =
...
module Diagnostics = struct
  ...
  let iter f diags =
    List.iter f (List.rev !diags)

  let format_all ?(use_color=true) diags =
    let buf = Buffer.create 1024 in
    iter (fun d ->
      Buffer.add_string buf (format_diagnostic ~use_color d);
      Buffer.add_char buf '\n'
    ) diags;
    Buffer.contents buf
end