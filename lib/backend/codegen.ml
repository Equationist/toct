...
module CodeGenerator (M: MACHINE) = struct
  ...
  let lower_function ~register_string_literal (func: Instructions.func) : machine_instr list * frame_info =
    (* Phase 1: SSA destruction *)
    let func' = destruct_ssa func in
    
    (* Phase 2: Instruction selection *)
    let machine_instrs = Selector.select_function ~register_string_literal func' in
  ...
  let format_machine_op ?(format_symbol=(fun x -> x)) op =
    ...
    | ADR (dst, label) -> Printf.sprintf "adr %s, %s" (fmt_reg dst) label
  ...
  let generate_module (m: Compilerkit_pir.Module_ir.pir_module) : string =
    ...
    (* Reset string literal table in backend *)
    M.string_literal_table := [];
    M.string_literal_counter := 0;
    ...
    List.iter (function
      | Module_ir.FuncDecl func ->
        ...
        let instrs, frame = lower_function ~register_string_literal:M.register_string_literal func in
    ...
    if !M.string_literal_table <> [] then begin
      Buffer.add_string output "\n";
      if is_macos && M.config.word_size = W64 then
        Buffer.add_string output ".section __TEXT,__cstring,cstring_literals\n"
      else
        Buffer.add_string output ".section .rodata\n";
      
      List.iter (fun (label, str) ->
        Buffer.add_string output (Printf.sprintf "%s:\n" label);
        Buffer.add_string output (Printf.sprintf "\t.asciz \"%s\"\n" 
          (String.escaped str))
      ) (List.rev !M.string_literal_table)
    end;
