...
module CodeGenerator (M: MACHINE) = struct
  ...
  let format_machine_instr (instr: machine_instr) : string =
    let label_str = match instr.label with
      | Some lbl -> Printf.sprintf "%s:\n" lbl
      | None -> ""
    in
    let op_str = "\t" ^ format_machine_op instr.op in
  ...
  let generate_module (m: Compilerkit_pir.Module_ir.pir_module) : string =
    ...
    List.iter (function
      | Module_ir.FuncDecl func ->
        ...
        (* Emit instructions *)
        List.iter (fun instr ->
          Buffer.add_string output (format_machine_instr instr);
      ...
      | Module_ir.ObjectDecl obj ->
        (* Global variable *)
        Buffer.add_string output (Printf.sprintf "\n.data\n");
        Buffer.add_string output (Printf.sprintf ".global %s\n" obj.obj_name);
        Buffer.add_string output (Printf.sprintf "%s:\n" obj.obj_name);
